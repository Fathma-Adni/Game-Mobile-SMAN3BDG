// =============================================
// SCENE MANAGER - TRUE HAPPINESS QUEST
// =============================================
...
class SceneManager {
    ...
    showScene(sceneId) {
        ...
        this.currentScene = document.getElementById(sceneId);
        if (this.currentScene) {
            ...
            if (this.isMobile) {
                window.scrollTo(0, 0);
                this.currentScene.scrollTop = 0;
            }
        }
    }
    
    transitionTo(sceneId, callback) {
        if (this.currentScene) {
            this.currentScene.classList.add('fade-out');
            setTimeout(() => {
                this.showScene(sceneId);
                if (callback) callback();
            }, 600); // Lebih cepat untuk mobile
        } else {
            this.showScene(sceneId);
            if (callback) callback();
        }
    ...
    // Mobile-specific method untuk swipe navigation
    setupSwipeNavigation() {
        ...
        document.addEventListener('touchend', (e) => {
            ...
            // Hanya proses jika swipe horizontal
...
const GameState = {
    ...
    lastChoiceTime: 0,          // Untuk throttle pilihan
...
function detectMobile() {
    ...
    sceneManager.isMobile = isMobile;
    console.log(`Mobile device detected: ${isMobile}, Screen width: ${window.innerWidth}`);
    return isMobile;
}
...
function setupMobileOptimization() {
    if (GameState.isMobile) {
        ...
        sceneManager.setupSwipeNavigation();
...
function setupViewportHeight() {
    ...
    const setVH = () => {
        ...
        // Juga update semua screen untuk memastikan height tepat
...
// =============================================
// MOBILE EVENT HANDLERS & UTILITIES
// =============================================
...
function setupScene() {
    ...
    // Setup untuk pilihan yang tidak terjangkau (Irma)
    disableUnaffordableChoices();
...
function setupMobileEventListeners() {
    ...
    choiceButtons.forEach(button => {
        ...
        button.removeEventListener('touchstart', handleChoiceTouchStart);
        button.removeEventListener('touchend', handleChoiceTouchEnd);
        button.removeEventListener('touchcancel', handleChoiceTouchCancel);
    ...
    const actionButtons = document.querySelectorAll('.start-btn, .select-btn, .restart-btn, .continue-btn');
    
    actionButtons.forEach(button => {
        // Hapus event listeners lama jika ada
        button.removeEventListener('touchstart', handleButtonTouchStart);
        button.removeEventListener('touchend', handleButtonTouchEnd);
        button.removeEventListener('touchcancel', handleButtonTouchCancel);
...
/**
 * Disable pilihan yang tidak terjangkau (khusus Irma)
 */
function disableUnaffordableChoices() {
    if (GameState.currentCharacter !== 'irma') return;
    ...
    choiceButtons.forEach(button => {
        ...
        // Jika membutuhkan uang dan tidak cukup
        if (effects.money < 0 && !canAffordChoice('irma', effects)) {
            button.disabled = true;
            button.style.opacity = '0.5';
            button.style.cursor = 'not-allowed';
            
            // Tambahkan tooltip untuk mobile
            if (GameState.isMobile) {
                button.title = `Butuh ${formatMoney(Math.abs(effects.money))}, kamu punya ${formatMoney(GameState.irma.money)}`;
            }
        }
    });
}
...
function handleChoiceTouchStart(e) {
    const now = Date.now();
    if (GameState.isProcessing || (now - GameState.lastChoiceTime < 500)) {
        e.preventDefault();
        return;
    }
...
function handleChoiceClick(e) {
    ...
    const transitionDelay = GameState.isMobile ? 1800 : 2200;
    setTimeout(() => {
...
function handleButtonTouchEnd(e) {
    ...
    // Trigger click dengan delay kecil
    setTimeout(() => {
...
function handleButtonClick(e) {
    ...
    switch(action) {
        case 'start':
            sceneManager.transitionTo('intro-screen', () => {
                console.log('Transitioned to intro screen');
            });
            break;
        case 'select-adni':
...
// =============================================
// PARTICLE EFFECT SYSTEM (OPTIONAL)
// =============================================

function createParticles(x, y, count = 10) {
    // Jangan buat particle efek di mobile untuk performa
    if (GameState.isMobile && count > 5) {
        count = 5;
    }
    ...
    for (let i = 0; i < count; i++) {
        ...
        // Gunakan setTimeout untuk performa di mobile
        if (GameState.isMobile) {
...
function setupLoadingScreen() {
    ...
    if (!document.getElementById('loading-screen')) {
        ...
        loadingScreen.innerHTML = `
            ...
            <p style="margin-top: 20px; color: var(--yellow-primary); font-size: 14px;">Memuat permainan...</p>
        `;
    ...
    window.addEventListener('beforeunload', () => {
        ...
        if (!loadingScreen) {
            ...
            newLoadingScreen.className = 'loading';
            newLoadingScreen.innerHTML = '<div class="loading-spinner"></div>';
...
// Setup scene jika ada scene yang aktif
document.addEventListener('DOMContentLoaded', () => {
    console.log('DOM fully loaded');
    
    // Setup scene jika kita berada di scene yang membutuhkan
    if (document.querySelector('.scene-content') || 
        document.querySelector('.character-select-screen') || 
        document.querySelector('.ending-screen')) {
        setupScene();
    }
    
    // Setup scene manager untuk single page version
    if (document.getElementById('intro-screen')) {
        sceneManager.showScene('intro-screen');
    }
});
...
window.addEventListener('orientationchange', () => {
    ...
    // Reset beberapa gaya setelah orientation change
    setTimeout(() => {
        setupViewportHeight();
    }, 100);
});
